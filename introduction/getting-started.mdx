...
Finally, we'll create a new SQL file in the `/src` directory called main2.sql, referencing main, and calling SDF's built in lineage command.
...
<Steps>
    <Step title="Static Analysis with `sdf compile`">
        First we'll try `sdf compile --show all`. The `--show` flag allows you to modulate SDF's output and the `all` option indicates that we would like to see all schemas from all models referenced in the workspace.
        ```shell
...
+-------------+------------------+------------+
   Finished 1 model [1 succeeded] in 0.177 secs
...
+----------+-----------------+
1 rows.
   Finished 1 model [1 succeeded] in 0.170 secs
...
└──────┐
       main.column_1
   Finished 2 models [2 succeeded] in 0.177 secs

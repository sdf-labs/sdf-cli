...
3. SDF introduces Code Reports and Code Contracts, e.g. queries that run against above information schema. A code contract might for example be that all tables specify their purpose.   
...
We **classify all root tables** as either machine or static or user data. A table that is classified as user data must have at least one column labeled as PII. We can write a code contract to ensure any table containing PII also contains a `user_data` table classifier.
...
### Writing our first code contract
...
In SDF we implement policies with code contracts against the information schema. Recall that we have three information schema: tables, columns, and partitions.
...
This table thus makes no sense. Let’s introduce a code contract that makes sure that at least all user tables are rerun within 90 days (the udf as_interval takes a list of [+-]d+[dwmy] strings and casts it as an interval).   

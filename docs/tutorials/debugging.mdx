...
└──────┐
    ...
    <Step title="Identify the Issue Upstream">
        ...
        ``` sql models/staging/stg_installs_per_campaign
        SELECT 
            campaign_id,
...
└──────┐
    ...
    <Step title="Update the Model">
        ...
        Update the `FROM` statement `stg_installs_per_campaign`, to the use `app_installs_v2`:
...
Working set 12 model files, 8 .sdf files
  ...
  Compiling moms_flower_shop.pub.dim_marketing_campaigns (./models/analytics/dim_marketing_campaigns.sql)
   Finished 12 models [3 succeeded, 9 reused] in 0.689 secs
...
└──────┐
    ...
    <Step title="Fix it!">
        To materialize the fix, all we need to do it run the queries:
...
<Steps>
    <Step title="Open the File">
        To keep it simple, we sill use `models/staging/stg_installs_per_campaign.sql`
  ...
  --> models/staging/stg_installs_per_campaign.sql:2:5
    ...
    <Step title="Error: Un-aggregated columns in aggregation context">
        ...
        SELECT 
            ...
            COUNT(event_id) AS total_num_installs
        FROM app_installs_v2
        GROUP BY 1  -- We didn't the new column here
        ```

...
`seed` | Seeds are models created from resources. The sdf local engine enables type checking of seeds independently of providers. Seeds can optionally be uploaded to the remote warehouse. To learn more: https://docs.sdf.com/reference/sdf-yml#enum-includetype|
...
Note that the difference between this dialect and [Dialect::Snowflake] is limited to non-SQL sources *only* (e.g. `.sdf.yml` files, parquet/CSV files). For SQL files, these two dialects are identical.

NOTE: the serialized form of this dialect is `snowflake`, for backward compatibility's sake:|
`snowflake_normalized` | Snowflake dialect with semantically accurate column names
...
NOTE: the serialized form of this dialect is `snowflake_normalized`, for backward compatibility's sake:|
## Enum: `Preprocessor`
Values |
| :--- |
`none`
...
`target-dir:` | `string?`| The dbt target to pass to undefineddbt parseundefined invocation|
...
`case-literal:` | [`Capitalization?`](#enum-capitalization)| Inconsistent capitalization of undefinedbooleanundefined/undefinednullundefined literal |
...
`no-these-identifiers:` | `[string]?`| |
`keywords-as-identifiers:` | `[string]?`| Keywords should not be used as identifiers |
`no-chars-in-quoted-identifiers:` | `string?`| Do not use special characters in identifiers |
`no-unnecessary-quoted-identifiers:` | [`RuleOn?`](#enum-ruleon)| Unnecessary quoted identifier |
`no-unqualified-multi-source-column-reference:` | [`RuleOn?`](#enum-ruleon)| Columns reference should specify source table or view in queries with more than one source. |
`no-inconsistent-ordinal-column-reference:` | [`RuleOn?`](#enum-ruleon)| Inconsistent column references in GROUP BY/ORDER BY clauses of both ordinal and non-ordinal |
`no-unnecessary-else:` | [`RuleOn?`](#enum-ruleon)| Redundant undefinedELSE NULLundefined in a case when statement |
`no-unnecessary-case:` | [`RuleOn?`](#enum-ruleon)| CASE statement can be simplified |
`no-unused-cte:` | [`RuleOn?`](#enum-ruleon)| Query defines a CTE (common-table expression) but does not use it |
`no-unnecessary-nested-case:` | [`RuleOn?`](#enum-ruleon)| Nested CASE statement in ELSE clause could be flattened. |
`no-distinct-parenthesis:` | [`RuleOn?`](#enum-ruleon)| DISTINCT used with parentheses |
`no-subquery-in:` | `[SubQueryScope]?`| Join/From clauses should not contain subqueries. Use CTEs instead |
`strict-join-condition-order:` | [`RuleOn?`](#enum-ruleon)| Join conditions column references should follow tables reference order |
`strict-wildcard-order:` | [`RuleOn?`](#enum-ruleon)| Select wildcards then simple targets before calculations and aggregates. |
`no-trailing-semicolon:` | [`RuleOn?`](#enum-ruleon)| Statements should not end with a semi-colon. Multi-statements must be separated with a semi-colon but the final statement should NOT end with one. |
`ctx-case-column:` | [`Capitalization?`](#enum-capitalization)| Inconsistent capitalization of column names |
...
`ctx-no-implicit-conversions-in:` | `[ImplicitConversionOperators]?`| Prevent type implicit casting or coercion |
`ctx-strict-qualified-column-reference:` | [`ColumnQualifier?`](#enum-columnqualifier)| Inconsistent column references in SELECT clause of both qualified and unqualified references |
`perf-only-direct-partition-column-access:` | [`RuleOn?`](#enum-ruleon)| Avoid functions in where clauses over indexed, partitioned or clustered columns, to prevent filter push down and improve performance |
`check-rules:` | `object?`| |

...
* `--log-form  LOG_FORM ` — Set the log format; default flat

  Possible values:
  - `flat`:
    Writes nd-json, compact, no nested objects
  - `nested`:
    Writes nd-json, compact, nested objects
  - `pretty`:
    Writes nd-json, pretty, nested objects

* `--log-filter  LOG_FILTER ` — Filters log messages; filters are written as  key : prefix_of_value 
* `--target-dir  TARGET_DIR ` — Set the  target-dir  for this session
...
* `--fetch` — Fetches changes to tables with remote origin

  Default value: `false`

  Possible values: `true`, `false`

* `--save-assembly` — Saves assembly.sdf.yml and sdf.information_schema.* in sdftarget/profile/...
...
* `--target-dir  TARGET_DIR ` — Set the  target-dir  for this session
* `--credentials-dir  CREDENTIALS_DIR ` — Read the credentials from  credentials-dir 
* `--prefer-local` — When set will minimize remote DB accessess and will rely on local schema information in .sdf.yml files

  Default value: `false`

  Possible values: `true`, `false`

* `--no-incremental-mode` — Set incremental-mode to false (deault is true)

  Default value: `false`

  Possible values: `true`, `false`

* `--no-snapshot-mode` — Set snapshot-mode to false (deault is true)
...
* `--exec-raw-query` — Hidden Option to Run Queries in Raw Mode
...
* `-u`, `--sql-logic-test-update` — Goldie update the sqllogictests files
...
* `--dialect  DIALECT ` — Dialect for all functions
  ...
  Possible values: `snowflake`, `trino`, `bigquery`, `redshift`, `spark-lp`
...
* `-n`, `--no-save` — DEPRECATED: this option has no effect; see --save

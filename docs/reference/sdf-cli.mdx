...
* `--dbt-target-dir  DBT_TARGET_DIR ` — A path to a dbt target directory
...
**Usage:** `sdf auth login bigquery [OPTIONS] --project-id  PROJECT_ID  --client-email  CLIENT_EMAIL  --private-key  PRIVATE_KEY `
...
* `--dialect  DIALECT ` — Dialect for all functions
  ...
  Possible values: `snowflake`, `trino`, `bigquery`, `redshift`, `spark-lp`, `sdf`, `databricks`

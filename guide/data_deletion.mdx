...
We present a method to fully automate data deletion and retention policies for GDPR compliance in a daily updated warehouse.

All versions require the following main components for automation:
...
3. SDF introduces Code Reports and Code Contracts, e.g. queries that run against above information schema. A code check might for example be that all tables specify their purpose.   
A code report, for example, can represent the partitions that have expired and thus should be deleted. 
...
We **classify all root tables** as either machine or static or user data. A table that is classified as user data must have at least one column labeled as PII. We can write a code check to ensure any table containing PII also contains a `user_data` table classifier.
...
In SDF we implement policies with code checks against the information schema. Recall that we have three information schema: tables, columns, and partitions.
...
**Deletion of partitions:** Here is a sql snippet for a code report that for given @date lists all partitions that should be deleted since they fall their retention period is expired.
...
This table thus makes no sense. Let’s introduce a code check that makes sure that at least all user tables are rerun within 90 days (the udf as_interval takes a list of [+-]d+[dwmy] strings and casts it as an interval).   

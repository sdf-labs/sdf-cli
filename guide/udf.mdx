...
User-defined functions allow to extend a database by adding a new function, that can be evaluated in SQL. The SQL standard distinguishes between scalar, aggregate and  
...
Section \cite() will explain how to author user defined functions.  

## Scalar functions

SDF distinguish scalar functions in various dimensions:
- They can have fixed or varying arity, i.e number oif parameters
- They can have one defintion or overloaded, i.e. have manay defintions.
- They can be monmorphic, i.e., defined for one type only, or polymorphic, i.e. defined to perate on all types
- They can be first-order, i.e., take only data as parameters, or higher order, take also functions as parameter.
We erxplain eac of these concepts and the authoring of the correspsondng function signature in turn.  

### Fixed Arity
The function `substr` ia a typical scalar function. It takes two arguments of the types `varchar` and `int`, respectively. 
...
Presto and Spark support higher order function applications. As an example, Presto's  `transform` function applies a function pointwise to an array,
e.g., the expression `transform(array[1,2,3], x -> x*x)` returns in Presto the value `array[1,4,9]`.
...
```
### Bounded types
Sometimes functions don;t work over all ttpes, but can only applied to a limited set of types. 
...
So to define `substr` as available for Presto we write: 
...
### Type signatures that depend on literals
Other spark functions change its type based on a provided literal. For instance

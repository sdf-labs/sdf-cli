...
4. They can be easily be set per run with the `--vars` flag on CLI commands like [`sdf compile`](/reference/sdf-cli#sdf-compile) and [`sdf run`](/reference/sdf-cli#sdf-run)
...
Next, to set the value of `@my_variable`, you can use the `--vars` flag on the CLI. For example:
```shell

...
<Steps>
    ...
    <Step title="Add the GitHub Action">
        ...
        1. **Dockerfile** - We can place this in the repository root, since this Docker image contains everything necessary to run SDF. If you're using DBT, use the contents in the `Dockerfile.dbt` file below. Otherwise, just use the `Dockerfile` contents below.
    ...
    ```
       ...
       <Info>
        ...
        Lastly, make sure to make the `entrypoint.sh` file executable by running the following command in your terminal:
        ```shell

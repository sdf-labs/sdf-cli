...
Begin by specifying the index type to `catalog-schema-table-name` and defining the `default-catalog` and `default-schema`:
```yaml workspace.sdf.yml
workspace:
   name: csv_123
   edition: "1.1"
...
important to add and update the `default-catalog` field within the `workspace.sdf.yml` file to reflect 
...
the folder directories to reflect this change. It is important to set the `default-catalog` on each 
of the includes paths. For example, add the `dafault-catalog` to each path, `src` and `local` below. 
If the `default-catalog` is not set in each path, the catalog will default to the workspace name.

...
<Note>
When attaching classifiers to tables, using the fully qualified name is required. For example, in an environment that uses catalog and schema names, the name must be listed as `<catalog-name>.<schema-name>.<table>`.
</Note>
...
Now we can run `sdf compile --show all` to see how classifiers are propagated into the

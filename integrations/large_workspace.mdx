...
## Prerequisites
 ...
 - Your compiled DBT output in the /target folder (see [dbt guide](/guide/dbt)). Or, your SQL transformations as files.
...
<Steps>
    <Step title="Successfully Load All DDLs">
        ...
        To validate that all DDLs are successfully ingested by SDF, run describe.
        ``` shell
	...
	<Step title="Add DBT Seed Tables (optional)">
        ...
        Run `sdf describe` to confirm that the seeds tables are properly represented in your schema.
        <Note>
        For Seeds, it's best to add their fully qualified name, including catalog and schema.
		</Note>
    </Step>
    <Step title="Add a Single Compiled Query">
        Now that you are able to run `sdf describe` on all ddls in your warehouse, it's time to add your transformations.
        ...
        To verify your query has been loaded successfully, run `sdf describe --no-show` and to see lineage, run `sdf lineage <dbt>/<target/[path]/<to>/transformation.sql`

        `sdf describe` should produce no errors.
    ...
    <Step title="Repeat for All Directories">
        Continue adding compiled directories to your includes, validating that each addition was successful with `sdf describe --no-show` producing no errors.
        ``` 
		includes:
            - path: ddls
              index: schema-table-name

...
<Steps>
    ...
    <Step title="Add Redshift Provider in Workspace">
        ...
        ```
        <Note>
            The `*` character is a wildcard (glob pattern). Specifying a database alone is shorthand for writing, `db1.*.*`. If you wanted to pull down any sources not found while compiling, you can just use `*.*.*`
        </Note>
    </Step>

...
<Note>
	`sdf dbt refresh` is run automatically when you run `sdf compile`, `sdf lineage`, and `sdf push`. Therefore, this command is rarely necessary since `sdf compile` will run off the latest DBT models.
</Note>

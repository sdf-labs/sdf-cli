...
## approx_set
```sql
function approx_set(bigint) returns varbinary
```


```sql
function approx_set(double) returns varbinary
```


```sql
function approx_set(varchar) returns varbinary
```
...
```sql
function bing_tile_coordinates(varchar) returns row(x int, y int)
```
...
```sql
function cardinality(varbinary) returns bigint
```
...
## empty_approx_set
```sql
function empty_approx_set() returns varbinary
```
...
## from_big_endian_32
```sql
function from_big_endian_32(varbinary) returns int
```
...
```sql
function geometry_from_hadoop_shape(varbinary) returns varchar
```
...
```sql
function geometry_nearest_points(varchar, varchar) returns row(c0 varchar, c1 varchar)
```
...
## merge
```sql
function merge(varbinary) returns varbinary
```
...
## now
```sql
function now() returns timestamp(3)
```
...
```sql
function qdigest_agg(real) returns varchar
```


```sql
function qdigest_agg(real, bigint) returns varchar
```


```sql
function qdigest_agg(real, bigint, double) returns varchar
```
...
```sql
function quantile_at_value(varchar, real) returns double
```
...
```sql
function sequence<$1> (date, date, $1) returns array[date]
...
## st_asbinary
```sql
function st_asbinary(varchar) returns varbinary
```
...
## st_geomfrombinary
```sql
function st_geomfrombinary(varbinary) returns varchar
```
...
## to_milliseconds
```sql
function to_milliseconds<$1> ($1) returns bigint
	where $1 in (interval day to second, interval year to month)
```

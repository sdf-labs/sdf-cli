...
| :---- | :--- | :------- |
`edition:` | `string`| The SDF edition, should always 1.2 (1.1. will soon ebe deprecated)|
...
`environments:` | `array`| Targets define input and output locations for artifacts read and written by the workspace|
...
`table-type:` | `TableType?`| The table-type of this table (obsolete)|
...
`creation-flag:` | `CreationFlags?`| Defines teh table creation options, defaults to none if not set|
...
`seeded:` | `boolean?`| Table is defined by these .sql and/or .sdf files Is this table backed by a DDL or a type provider|
`source-locations:` | `array`| |
## Block: `Classifier`
|Field | Type | Description|
| :---- | :--- | :------- |
...
Value | Description|
| :--- | :------- |
`code-report` | undefined|
`system` | A System table/view, maintained by the database system|
`code-contract` | A code contract|
`data-contract` | A data contract|
`data-report` | A data report|
## Enum: `Variadic`
Value | Description|
| :--- | :------- |
...
|Field | Type | Description|
| :---- | :--- | :------- |
`profile:` | `string?`| The default profile (can only be set on the level of the workspace)|
`environment:` | `string?`| The default environment (can only be set on the level of the workspace/profile)|
`dialect:` | `Dialect?`| The dialect of this profile. If not set, defaults to presto|
`compute:` | `ComputeKind?`| The compute platform for this profile. If not set, defaults to local|
`preprocessor:` | `Preprocessor?`| The compute platform for this profile. If not set, defaults to local|
...
`creation-flag:` | `CreationFlags?`| Defines table creation flags, defaults to  if not set|
...
`branch:` | `string?`| the Git branch (choose only one of the fields: rev, branch, tag)|
`tag:` | `string?`| the Git tag (choose only one of the fields: rev, branch, tag)|
### Nested element: `TargetEnvironment`
|Field | Type | Description|
| :---- | :--- | :------- |
`name:` | `string`| The name of the target|
`description:` | `string`| A description of this target|
`providers:` | `array`| The providers for this target|
`defaults:` | `Defaults?`| Defaults for this target|
### Nested element: `TargetProvider`
|Field | Type | Description|
| :---- | :--- | :------- |
`scopes:` | `array`| A list of scopes supported by this provider|
...
`source-locations:` | `array`| |
`batch-size:` | `integer?`| |
### Nested element: `ProviderScope`
|Field | Type | Description|
| :---- | :--- | :------- |
`name:` | `string`| Matches a three part table name using globs, e.g. *.*.* matches all tables|
`as:` | `string`| Compose a new table, use * to back reference the matched name, e.g. *.&#123;&#123;env_var(&#39;USER&#39;&#125;)&#125;&#125;_*.* renames all tables to have the &#96;USER&#96; as a prefix of every schema|
...
`default-schema:` | `string`| Defines a default schema (If not set, defaults to &#39;pub&#39;)|
`dependencies:` | `array`| Dependencies of the workspace to other workspaces or to cloud database providers|
`environments:` | `array`| Targets define input and output locations for artifacts read and written by the profile|
`default-target:` | `string?`| The default target for this profile|
`vars:` | `object`| A map of named values for setting SQL variables from your environment Ex. -dt: dt, used in SQL as @dt, and in Jinja as \&#123;\&#123; dt \&#125;\&#125;|
`default-profile:` | `string`| Defines the default profile (if not set, defaults to &#39;dbg&#39;)|
`source-locations:` | `array`| Workspace defined by these set of files|
...
`is-dbt-project:` | `boolean?`| |
### Nested element: `TableLocation`
|Field | Type | Description|
| :---- | :--- | :------- |

...
`dialect:` | `Dialect?`| The dialect of this environment. If not set, defaults to presto|
`compute:` | `ComputeKind?`| The compute platform for this environment. If not set, defaults to local|
...
|Field | Type | Description|
| :---- | :--- | :------- |
`namespaces:` | `array`| A list of namespaces supported by this provider|
...
`batch-size:` | `integer?`| |
### Nested element: `ProviderNamespace`
|Field | Type | Description|
| :---- | :--- | :------- |
`target:` | `string`| Matches a three part table name using globs, e.g. *.*.* matches all tables|
`rename-as:` | `string?`| Compose a new table, use * to back reference the matched name, e.g. *.&#123;&#123;env_var(&#39;USER&#39;&#125;)&#125;&#125;_*.* renames all tables to have the &#96;USER&#96; as a prefix of every schema|

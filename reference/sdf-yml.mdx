...
`default-target:` | `string?`| The default target for this workspace|
...
`type:` | `TableType2?`| The table-type of this table (new version)|
`table-modifier:` | `TableModifier?`| undefined|
`dependencies:` | `array`| ALl table dependencies (syntax: catalog.schema.table)|
`create-options:` | `TableCreateOptions?`| Specify how to create a table|
...
`system` | System Tables/ Views, like external tables but maintained by database|
`temporary` | Temporary -- per session tables|
`transient` | Transient table|
## Enum: `TableCreateOptions`
Value | Description|
| :--- | :------- |

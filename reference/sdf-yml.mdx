...
`excludes:` | `[ExcludePath]?`| An array of directories and filenames to be skipped when resolving includes|
...
`data-asset` | Contains data definition or data query|
`resource` | Contains resources used by the application (e.g. static csv files)|
## Enum: `Dialect`
Value | Description|
| :--- | :------- |
`presto` | undefined|
`snowflake` | SparkSQL Dialect Snowflake Dialect|
## Enum: `ExcludeType`
Value | Description|
| :--- | :------- |
`timeouts` | undefined|
`path` | Contains data definition or data query|
...
`code-check` | Code check query|
`data-check` | Data check query|
`code-report` | Code report query|
`access-check` | Access check query|
...
`include-type:` | `IncludeType?`| Type of included artifacts|
...
`lineage:` | `Lineage?`| Lineage, a tagged array of column references|
`reclassify:` | `[Reclassify]?`| Array of reclassify instructions for changing the attached classifier labels|
`samples:` | `[string]?`| An array of representative literals of this column [experimental!]|

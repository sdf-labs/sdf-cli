...
`variables:` | `array`| An array of named values for setting SQL variables from your environment Ex. -dt: dt, used in SQL as @dt|
...
`special:` | `boolean`| Function can be called without parentheses, e.g. current_date|
## Enum: `IncludeType`
Value | Description|
| :--- | :------- |
`model` | Contains models, i.e ddls or select statements, used for data transformation|
`test` | Contains static and dynamic test queries, the check holds if the data returns zero rows|
`stats` | Contains queries for data exploration, data profiling, data description and summarization|
`resource` | Contains resources used by the application, like static .csv files|
`metadata` | Contains meta data like classifiers, etc|
`code-check` | INTERNAL USE ONLY, temporary, will be removed|
`data-check` | INTERNAL USE ONLY, temporary, will be removed|
`code-report` | INTERNAL USE ONLY, temporary, will be removed|
`data-report` | INTERNAL USE ONLY, temporary, will be removed|
`access-check` | INTERNAL USE ONLY, temporary, will be removed|
## Enum: `SyncType`
Value | Description|
| :--- | :------- |

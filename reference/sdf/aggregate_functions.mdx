...
## approx_median
```sql
function approx_median<$1> (values $1) returns double
	where $1 in (bigint, int, double)
```
...
```sql
function approx_percentile_cont<$1> (values $1, p double) returns double
	where $1 in (bigint, int, double, float)
```
...
```sql
function approx_percentile_cont_with_weight<$1> (values $1, p double, weight float) returns double
	where $1 in (bigint, int, double)
```
...
## count_distinct
```sql
function count_distinct(values $1) returns double
```
...
## median
```sql
function median<$1> (values $1) returns double
	where $1 in (bigint, int, double, float)
```
...
function min<$1> ($1) returns $1
	where $1 in (bigint, int, float, double)
```

...
```sql
function !=<$1, $2> (<$1, $2>) returns boolean
	where $1 in (variant, varchar)
	and $2 in (variant, varchar)
```
...
function +<$1, $2> (<$1, $2>) returns $1
	where $1 in (date, time, timestamp)
	and $2 in (interval)
```
...
```sql
function <(<variant, variant>) returns boolean
```
...
```sql
function automatic_clustering_history(<>) returns struct<
  start_time timestamp,
  end_time timestamp,
  table_name string,
  credits_used bigint,
...
```sql
function convert_timezone(<varchar, varchar, timestamp>) returns timestamp
```
Returns the input timestamp converted from source timezone to a destination timezone

```sql
function convert_timezone(<varchar, timestamp>) returns timestamp
```
Returns the input timestamp converted from source timezone to a destination timezone
...
```sql
function database_replication_usage_history(<>) returns struct<
  start_time timestamp,
  end_time timestamp,
  database_name string,
  credits_used string,
...
```sql
function database_storage_usage_history(<>) returns struct<usage_date date,
  database_name string,
...
```sql
function dayname([date]) returns varchar
```
Day name
...
```sql
function login_history(<>) returns struct<event_timestamp timestamp, event_id bigint, event_type string, user_name string, client_ip string, reported_client_type string, reported_client_version string, first_authentication_factor string, second_authentication_factor string, is_success string, error_code bigint, error_message string, related_event_id bigint, "connection" string>
```
...
```sql
function login_history_by_user(<>) returns struct<event_timestamp timestamp, event_id bigint, event_type string, user_name string, client_ip string, reported_client_type string, reported_client_version string, first_authentication_factor string, second_authentication_factor string, is_success string, error_code bigint, error_message string, related_event_id bigint, "connection" string>
```
...
```sql
function materialized_view_refresh_history(<>) returns struct< start_time timestamp, end_time timestamp, credits_used string, materialized_view_name string >
```
...
```sql
function monthname([date]) returns varchar
```
Month name
...
```sql
function next_day(<date, varchar>) returns date
```
returns the first weekday named by weekday that is later than date
...
```
Returns the 1st input, with values equal to the 2nd input replaced with NULL
...
```sql
function object_construct($1, ...) returns object
```
the resulting object
...
```sql
function object_construct(<$1, $2>) returns object
```


```sql
function object_construct(<$1, $2, $3>) returns object
```


```sql
function object_construct(<$1, $2, $3, $4>) returns object
```


```sql
function object_construct(<$1, $2, $3, $4, $5>) returns object
```
...
```sql
function previous_day(<date, varchar>) returns date
```
returns the last weekday named by weekday that is earlier than date
...
```sql
function query_acceleration_history(<>) returns struct<start_time timestamp,
  end_time timestamp,
  credits_used bigint,
...
```sql
function query_history(<>) returns struct<query_id string,
  query_text string,
  database_name string,
  schema_name string,
  query_type string,
...
```sql
function query_history_by_session(<>) returns struct<query_id string,
  query_text string,
  database_name string,
  schema_name string,
  query_type string,
...
```sql
function query_history_by_user(<>) returns struct<query_id string,
  query_text string,
  database_name string,
  schema_name string,
  query_type string,
...
```sql
function query_history_by_warehouse(<>) returns struct<query_id string,
  query_text string,
  database_name string,
  schema_name string,
  query_type string,
...
```sql
function replication_usage_history(<>) returns struct<
  start_time timestamp,
  end_time timestamp,
  database_name string,
  credits_used string,
...
```sql
function search_optimization_history(<>) returns struct<
  start_time timestamp,
  end_time timestamp,
  credits_used string,
  table_name string
...
```sql
function serverless_task_history(<>) returns struct<
  start_time timestamp,
  end_time timestamp,
  task_name string,
  credits_used string
...
```sql
function stage_directory_file_registration_history(<stage_name string>) returns struct<
  job_created_time timestamp,
  file_name string,
  operation_status string,
  message string,
...
```sql
function stage_storage_usage_history(<>) returns struct<usage_date date, average_stage_bytes bigint>
...
```sql
function warehouse_load_history(<>) returns struct<start_time timestamp,
  end_time timestamp,
  warehouse_name string,
...
```sql
function warehouse_metering_history(<>) returns struct<start_time timestamp,
  end_time timestamp,
  warehouse_name string,
  credits_used bigint,

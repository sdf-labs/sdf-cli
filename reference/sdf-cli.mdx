...
* `--execute <EXECUTE>` — When to execute this job run; defaults to recurring: if --from/--to are given
  ...
  Possible values:
  - `once-immediately`:
    Executes the target immediately, disrespecting the target's schedule, if any
  - `once-on-last-landed`:
    Executes the target at the last time when all data for the targets time grain was available
  - `once-on-next-landing`:
    Executes the target once at the next possible time when all data for the targets time grain is available
  - `recurring`:
    Executes the target on schedule starting with --from (default is now) until --to or, if not given, forever
  - `no-execute`:
    Only the Metadata is deployed to the console, no runs are scheduled

* `--dry-run` — Create a deploy.json and archive.tar.gz but don't post it to the service

  Default value: `false`
* `--no-exec` — Whether to schedule tasks to run, or just deploy metadata

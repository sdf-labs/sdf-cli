...
* `--format  FORMAT ` — Show tables in this format [ony table, json and yml supported]

  Default value: `table`

  Possible values: `table`, `csv`, `tsv`, `json`, `nd-json`, `yml`

* `--limit  LIMIT ` — Limiting number of shown columns. Run with --limit 0 to remove limit
...
* `--format  FORMAT ` — Show tables in this format
...
* ` TARGETS ` — Test models restricted to models and tests found in the given source dirs, files or tables [default: all models and all contracts]
...
* `-x`, `--experimental` — Modern output
  ...
  Default value: `false`
* `-s`, `--show  SHOW ` — Display messages [default: all]
...
* ` TARGETS ` — Run reports defined by the given source dirs, files or tables [default: all reports]

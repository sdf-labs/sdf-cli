...
* [`sdf dbt`↴](#sdf-dbt)
* [`sdf dbt build`↴](#sdf-dbt-build)
* [`sdf dbt test`↴](#sdf-dbt-test)
* [`sdf dbt run`↴](#sdf-dbt-run)
* [`sdf dbt compile`↴](#sdf-dbt-compile)
...
* `push` — Push a local workspace to the SDF Console
* `pull` — Pull a remote workspace from the SDF Console
...
* `--no-cache` — Don't read and write cached table data
  ...
  Default value: `false`
* `--sync-cache` — Delete irrelevant data from .sdfcache (tables, classifier, function, config)
...
Push a local workspace to the SDF Console

**Usage:** `sdf push [OPTIONS] [INPUT_FILES]...`
...
Pull a remote workspace from the SDF Console

**Usage:** `sdf pull [OPTIONS] [INPUT_FILES]...`
...
Run a query against your current workspace
...
* `<INPUT_FILES>` — Add these files/or directories to the selected workspace/profile

###### **Options:**

* `--profile <PROFILE>` — Add the files using this profile
* `--workspace-path <WORKSPACE_PATH>` — Add these files to this workspace (defaults to current working dir or any of its parents*)
* `--includes-path <INCLUDES_PATH>` — Add them to this include path (default folder is ./src)
* `--dialect <DIALECT>` — Assume this dialect for all files (default dialect is the workspace dialect)

  Possible values: `snowflake`, `presto`, `bigquery`, `redshift`, `spark-lp`

* `--default-catalog <DEFAULT_CATALOG>` — Assume this default catalog for all files (default dialect is the workspace dialect)
* `--default-schema <DEFAULT_SCHEMA>` — Assume this default schema for all files (default dialect is the workspace dialect)
* `--quiet` — Do not print sdf console messages
...
Infer artifacts like classifiers and add them optionally to the workspace
...
**Usage:** `sdf ai classify [OPTIONS] [INPUT_FILES]...`

###### **Arguments:**

* `<INPUT_FILES>` — Scope inference to these *.sql files; providing no input or a path to workspace.sdf.yml infers something for the whole workspace

###### **Options:**

* `--profile <PROFILE>` — Scope inference to using this profile
* `--save-location <SAVE_LOCATION>` — Add inferred classifiers to this include path (default folder is ./src)
* `--classifiers <CLASSIFIERS>` — Infer these classifiers (--classifier can be repeated), no classifier means all classifiers in scope
* `--no-use-of-samples` — Do NOT pass samples to inference engine

  Default value: `false`
* `--no-use-of-data` — Do NOT pass data values (at most 5 values randomly chosen) to inference engine

  Default value: `false`
* `--no-use-of-description` — Do NOT pass table/column descriptions (just first sentence) to inference engine
...
Run Dbt commands via sdf

**Usage:** `sdf dbt <COMMAND>`

###### **Subcommands:**

* `build` — Corresponds to dbt build: runs models, test tests, snapshot snapshots, seed seeds
* `test` — Corresponds to dbt test: runs tests defined on models, sources, snapshots, and seeds
* `run` — Corresponds to dbt run: executes compiled sql model files against the current target database
* `compile` — Corresponds to dbt compile:  generates executable SQL from source model, test, and analysis files (and puts it into target)
* `init` — Init
...
Corresponds to dbt build: runs models, test tests, snapshot snapshots, seed seeds

**Usage:** `sdf dbt build [OPTIONS] [INPUT_FILES]...`

###### **Arguments:**

* `<INPUT_FILES>` — corresponds to selectors in dbt: just give a list of directories or files, e.g. models, models/my_model.sql, models/my_model.yml

###### **Options:**

* `--profile <PROFILE>` — Scope inference to using this SDF profile
...
* `--save-assembly` — Save assembly in yaml format
  ...
  Default value: `false`
* `--vars <VARS>` — Supply variables to the project. This argument should be a binding of the form name=value, this argument can be supplied multiple times
* `--quiet` — Do not print sdf console messages
...
Corresponds to dbt test: runs tests defined on models, sources, snapshots, and seeds

**Usage:** `sdf dbt test [OPTIONS] [INPUT_FILES]...`
...
Corresponds to dbt run: executes compiled sql model files against the current target database

**Usage:** `sdf dbt run [OPTIONS] [INPUT_FILES]...`
...
Corresponds to dbt compile:  generates executable SQL from source model, test, and analysis files (and puts it into target)

**Usage:** `sdf dbt compile [OPTIONS] [INPUT_FILES]...`

...
error: : workspace: unknown field `code-checks`, expected one of `edition`, `name`, `description`, `repository`, `remote-location`, `includes`, `excludes`, `references`, `dialect`, `default-catalog`, `default-schema`, `default-profile`, `source-locations`, `variables`
  --> workspace.sdf.yml:15:3
   Finished describe in 0.003 secs
